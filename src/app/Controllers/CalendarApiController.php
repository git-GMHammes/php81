...
class CalendarApiController extends ResourceController
{
    use ResponseTrait;
    ...
    public function dbRead($parameter1 = NULL, $parameter2 = NULL, $parameter3 = NULL)
    {
        ...
        $uploadedFiles = $request->getFiles();
        $json = $processRequest['json'] ?? 0;
        $data['ano'] = ($parameter1 !== NULL) ? ($parameter1) : (date('Y'));
        $data['mes'] = ($parameter2 !== NULL) ? ($parameter2) : (date('m'));
        $data['dia'] = ($parameter3 !== NULL) ? ($parameter3) : (date('d'));
        $processRequest = $data;
        // myPrint($data, 'src\app\Controllers\CalendarApiController.php');
        // $processRequest = eagarScagaire($processRequest);
        # 
        try {
            if (isset($processRequest['id'])) {
